...
1. **자유 집필 → 유도 집필** — 기·승 단계는 아이가 AI와 한 문단씩 함께 쓰고(작가), 전·결 단계는 AI가 결말을 비밀로 한 챕터씩 펼친다(첫 독자). 전환은 매끄럽다.
...
키 없이 실행하면 인메모리 저장소 + 결정적 mock AI로 전체 흐름이 동작한다. 실 서비스는 Supabase·Google 키를 채운다(→ `documents/instructions/07-운영/01-실행-배포.md`).

---

## 문서

프로젝트의 모든 상세 문서는 **[`documents/instructions/`](documents/instructions/README.md)** 에 있다 — 서비스 개요, 아키텍처, 역할별 기능, API 레퍼런스, 작동 흐름, 디자인 시스템, 운영/배포. 무엇이 어디 있는지는 [instructions/README.md](documents/instructions/README.md) 참고.
